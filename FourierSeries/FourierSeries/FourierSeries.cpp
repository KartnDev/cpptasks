...
csignal fourier_transform(const csignal &s, bool back = false)
{
	...
	const double pol{ 2.0 * PI * (back ? -1.0 : 1.0) };
	...
	auto to_ft([=, &s](size_t j) {
		return accumulate(num_iterator{ 0 }, num_iterator{ s.size() }, cmplx{}, sum_up(j));
		});
...
{
	return[period_len, n{ 0 }]() mutable { return cos(double(n++) * 2.0 * PI / period_len); };
}


static auto gen_square_wave(size_t period_len)
{
	return[period_len, n{ period_len * 7 / 4 }]() mutable { return ((n++ * 2 / period_len) % 2) * 2 - 1.0; };
}
